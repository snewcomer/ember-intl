...
class TranslationReducer extends CachingWriter {
  ...
  readDirectory(inputPath, listFiles) {
    /**
     * sorted so that any translation path starts with `addon`
     * move to the head of the array.  this ensures the application's translations
     * take presidence over addon translations.
     */
    let sortedPaths = listFiles.sort(filepath => {
      if (path.relative(inputPath, filepath).startsWith('addons')) {
        return -1;
      }
    ...
    return sortedPaths.reduce((translations, filepath) => {
  ...
  build() {
    ...
    for (let locale in translations) {
      if (translations.hasOwnProperty(locale)) {
        ...
        if (this.options.verbose) {
          if (baseLocale && defaultTranslationKeys) {
